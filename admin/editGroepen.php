...
if(isset($_POST['save'])) {
	...
	if(mysqli_query($db, $sql)) {
		...
		toLog('info', '', 'Groep '. $_POST['naam'] .' '. ($actie == 'add' ? 'toegevoegd' : 'gewijzigd'));
	} else {
		$text[] = "Probleem met opslaan groep";
		toLog('error', '', 'Kon groep '. $_POST['naam'] .' niet '. ($actie == 'add' ? 'toevoegen' : 'wijzigen'));
	}
	...
	if(mysqli_query($db, $sql_groep)) {
		...
		toLog('info', '', 'Groep '. $groepData['naam'] .' verwijderd');
	} else {
		$text[] = "Probleem met verwijderen groep <i>". $groepData['naam'] ."</i><br>";
		toLog('error', '', 'Kon groep '. $groepData['naam'] .' niet verwijderen');
	}
	...
	if(mysqli_query($db, $sql_leden)) {
		...
		toLog('debug', '', 'leden van '. $groepData['naam'] .' verwijderd');
	} else {
		$text[] = "Probleem met verwijderen leden van <i>". $groepData['naam'] ."</i><br>";
		toLog('error', '', 'Kon leden van '. $groepData['naam'] .' niet verwijderen');
	}
	...
	if(mysqli_query($db, $sql_rooster)) {
		...
		toLog('debug', '', 'rooster met '. $groepData['naam'] .' verwijderd');
	} else {
		$text[] = "Probleem met verwijderen rooster met <i>". $groepData['naam'] ."</i><br>";
		toLog('error', '', 'Kon rooster met '. $groepData['naam'] .' niet verwijderen');
	}
