...
if(isset($_POST['save'])) {
	...
	if(mysqli_query($db, $sql)) {
		...
		toLog('info', '', 'Rooster '. $_POST['naam'] .' '. ($actie == 'add' ? 'toegevoegd' : 'gewijzigd'));
	} else {
		$text[] = "Probleem met opslaan rooster";
		toLog('error', '', 'Kon rooster '. $_POST['naam'] .' niet '. ($actie == 'add' ? 'toevoegen' : 'wijzigen'));
	}
	...
	if(mysqli_query($db, $sql)) {
		...
		toLog('info', '', 'Rooster '. $roosterData['naam'] .' verwijderd');
	} else {
		$text[] = "Probleem met verwijderen rooster <i>". $roosterData['naam'] ."</i><br>";
		toLog('error', '', 'Kon roosterData '. $roosterData['naam'] .' niet verwijderen');
	}	
