...
# Default is vanaf 26 uur geleden tot 1 minuut voor 12 deze dag
...
$logins = Login::getLogins($lid, $ip, "$bJaar-$bMaand-$bDag $bUur:$bMin:00", "$eJaar-$eMaand-$eDag $eUur:$eMin:59");

if(count($logins) > 0) {
	foreach($logins as $loginID) {
		$entry = new Login($loginID);
		if($entry->lid != '' && $entry->lid > 0) {
			$user = new Member($entry->lid);
		}
				
		$rij = array();
		$rij[] = "<tr>";
		$rij[] = "	<td>". $entry->tijdstip ."</td>";
		$rij[] = "	<td>&nbsp;</td>";
		$rij[] = "	<td>". ($entry->lid != '' ? "<a href='?lid=". $entry->lid ."'>". $user->getName(5) ."</a>" : "&nbsp;") ."</td>";
		$rij[] = "	<td>&nbsp;</td>";
		$rij[] = "	<td>". ($entry->ip != '' ? "<a href='https://who.is/whois-ip/ip-address/". $entry->ip ."'>". $entry->ip ."</a>" : "&nbsp;") ."</td>";
		$rij[] = "	<td>&nbsp;</td>";
		$rij[] = "	<td>". $entry->agent ."</td>";
		$rij[] = "</tr>";
		
		$rijen[] = implode(NL, $rij);
		
		if(!in_array($entry->lid, $users)) {
			$users[] = $entry->lid;
		}		
	}
}
...
foreach($users as $userID) {
	...
	$zoekScherm_2[] = "	<option value='$userID'". ($lid == $userID ? ' selected' : '') .">". $person->getName(5) ."</option>";
}
